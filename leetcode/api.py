...
def synthesize_problem_json(problem_json, params, isToSort=True):
    ...
    if params["difficulty"]:
        synthesized_problems = [e for e in synthesized_problems if e["difficulty"] == params["difficulty"]]

    if isToSort:
        # Sort by difficulty follwed by problem id in ascending order
        synthesized_problems = sorted(synthesized_problems, key=lambda x: (x["id"]))
